...
page('Chapter 11 ─ Context & Data',
     ...
     'RAGが検索する「知識」も、エージェントが認識する「状況」も、元をたどれば組織の'
     'データとコンテキストに行き着く。ここが薄ければ、上に積んだ仕組みは正しく機能しない。',
     fig_ladder([
         ('運用・ガバナンス', '更新のルールと、責任者を先に決めておく'),
         ('AIツールの活用', 'システムプロンプトや制約を設計する'),
         ('暗黙知の形式知化', '業務マニュアルや議事録を、AIが読める形にする'),
         ('動的コンテキスト', 'CRMやカレンダー、Slackなど最新のやり取り'),
         ('データ基盤（器）', '共有フォルダ・ナレッジベース・RAG用データの整備'),
     ], '', '', asc=True),
     [('<span class="tag">土台</span>「データ基盤（器）」が欠けると、上に何を積んでも崩れる',
       'RAGが検索する断片も、エージェントが認識する状況も、<b>すべてこの器から汲み上げている</b>。'),
      ('<span class="tag">正本</span>「これが正しい」と決めた1つの版が要る',
       '正本が複数ある組織では、AIは古い版を正確に読むだけになる。'
       '<b>ハルシネーションに見える誤答の何割かは、実はここが原因</b>。'),
      ('<span class="tag">詳しくは</span>コンテキスト＆ハーネスエンジニアリングへ',
       '何を渡すか（コンテキスト）と、どう動かすか（ハーネス）の設計は、'
       'それ自体が独立した技術になっている。')],
     src='出典: おざけん資料アーカイブ「コンテキスト＆ハーネスエンジニアリング」にもとづく（簡略版）。')
...
page('Chapter 11 ─ Careers',
     ...
     [('<span class="tag">何が沈むか</span>記述できる仕事から、順に代替されていく',
      ...
      ('<span class="tag">残るもの</span>目的を持つ人から、順に代替されない',
       ...
       '<b>目的さえあれば、Howの大半はAIが埋めてくれる</b>。'),
      ('<span class="tag">詳しくは</span>AIエージェント時代のキャリアへ',
       '日本でも同じ順番の変化が数字に出はじめている。裸の戦いで何が残るかは、'
       '別の資料でさらに詳しく扱っている。')],
     navy=True,
