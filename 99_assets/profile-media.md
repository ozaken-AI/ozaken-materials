...
通常のトップページを開いただけでは、今回追加した写真・講座画像・名簿の6素材は取得しません。`pr` でプロフィールを表示すると、その章と前後1章の画像から取得します。以降は移動した章の前後を読み込み、読み込んだ画像を再利用します。設定 JSON はトップページの初期表示時に取得します。

HTML では枠に `data-profile-media="lecture"` と `data-profile-media-frame` を付け、内側に次のように画像を置きます。画像枠は `.pd-chapter` 内に置いてください。
