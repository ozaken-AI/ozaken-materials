...
資料の作成・修正から公開後の確認までを、特定のAIや端末に依存せず引き継ぐための入口です。最終更新：2026-09-15。
...
本人のQR読み取り・質問とコメントの投影サイズの改善依頼により、`#askqr` のCSSと受信スクリプトを `99_assets/qa-stage.css`・`qa-stage.js` に分離した。同日の追加依頼で、角張った枠を柔らかい外枠と投稿カードへ改め、背景と受信更新の動きを整理した。変更対象は投影Q&Aのみ。トップの他のオーバーレイや、登壇者専用の `inbox.html`、投稿用の `ask.html` は変更対象外とする。

- テンプレートの深い紺・紙色・明朝の見出し・ゴシックの本文を使う。PCでは左に白いQRの外枠、右に薄い紺の質問欄と紙色の独立した投稿カードを置き、幅900px以下では縦に並べる。通常画面の角丸はQR外枠36px・質問欄34px・投稿23px。低い投影画面や小画面では角丸と内側余白を調整し、文字を小さくして詰め込まない。
...
- QRは外枠だけを丸め、SVGは正方形のまま白い余白の内側に置く。外枠のpadding分を寸法へ加え、QR本体の大きさを保つ。SVGの設計寸法は1440×900で約414px、1920×1080で464px、1280×720で約310px。スマートフォンは最大300px。外枠の幅を読み取りに使うコードの幅と混同せず、実表示でも確認する。拡大表示はEnter・Spaceでも開け、Escで元へ戻る。QRと本文は初めから完成状態で静止して表示する。
- 質問・コメントはPCで26〜42px、スマートフォンで22px。投稿の改行を保ち、長いURLも折り返す。省略・行数制限をしない。PCは質問欄をスクロールし、小画面は画面全体を縦に読む。
- 背景は投影Q&A専用の `.aq-ambient` に置く2層のCSSの光とする。従来のcanvas描画を使わず、28秒・34秒の周期でtransformとopacityだけを変える。QR・文字・投稿カードは動かさない。`#askqr.show` の表示中、タブ可視、動きを減らす設定が無効、の3条件を満たす場合だけ `.aq-motion-active` を付けて再生する。画面を閉じたとき・タブ非表示時は停止し、動きを減らす設定では光を静止、印刷では投影Q&A全体を非表示にする。
- 投影中の文字を自動で流さない。受信内容が変わらない場合は投稿DOMを作り直さず、同じノードを再利用する。投稿の識別には日時・本文・名前と同一投稿の出現順を使い、短い本文の一致だけで別の質問をまとめない。新しい投稿や表示順の変更があっても、PCでは質問欄、小画面では画面全体の読書位置を維持する。
- 更新間隔9秒、1〜3時間の表示範囲、既存の取得先とfetch／JSONPの経路を維持。範囲の切り替えや質問欄の操作で全画面の状態を誤って変えない。画面を閉じたとき・タブ非表示時は定期取得を止める。表示状態・タブ可視・動きを減らす設定の監視は一度だけ登録し、状態変更のたびにリスナーを増やさない。
- `index.html` のCSS／JS参照の `?v=` は各ファイルのSHA-256先頭12文字。内容を変えたら両方を更新する。リポジトリ外の架空投稿で0件・長文・多数・表示範囲・受信追加を検査し、テスト投稿を本番の受信箱へ送らない。

最初の改修の確認範囲と本番記録は [Q&A投影の改修記録](verification/2026-09-15-qa-stage.md)。今回の外観・背景・受信更新の確認は [Q&Aデザイン仕上げの記録](verification/2026-09-15-qa-polish.md) を参照する。
