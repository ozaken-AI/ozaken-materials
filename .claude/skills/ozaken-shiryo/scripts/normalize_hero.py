...
**2本だけ、わざと外れている。** `01_concept/ax-article.html` は記事の組み、
`05_drive/ai-51-master.html` は1枚もののポスターで、どちらも `.hero-title` を
持たない別の作りになっている。ここを表紙と同じ大きさにすると版が崩れるので、
この塊は当たらないままでよい（当ててもセレクタが一致しないので実害はない）。
