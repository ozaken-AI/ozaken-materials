...
/* 見出しの強調。**赤ではなく淡い青に下線。**
   赤で抜くと警告に見えて、題そのものより先に目が行ってしまう。
   淡い青に下線なら、題の一部として読まれたまま、そこだけ残る。
   点滅の演出も外した。投影中に見出しがちらつくと、話が入らない */
.hero-title .hl{color:var(--azure-pale);
  background-image:linear-gradient(var(--azure),var(--azure));
  background-size:100% 3px;background-position:0 calc(100% - .1em);
  background-repeat:no-repeat;padding-bottom:.12em}
...
/* システム表記バッジ（index と同じ）。
   **絶対配置ではなく、フローの中の1段目に置く。**
   以前は position:absolute で扉に重ねていたため、扉の中身（.inner）は
   バッジの存在を知らずに縦中央寄せされていた。タイトルが短い・
   ウィンドウが低いなどで .inner が伸びきらないと、中央寄せの結果が
   バッジのすぐ下まで浮き上がり、`apply_home.py` の名札（.oz-home）と
   文字が重なっていた（実際、ノートPCの高さでは常態化していた）。
   flexアイテムとして並べておけば、.inner はバッジと名札を除いた
   残りの領域でしか中央寄せされないので、どんな高さでも重ならない */
.sys-badge{position:relative;z-index:4;align-self:flex-start;flex:0 0 auto;
...
@media(max-width:640px){
  ...
  .sys-badge{margin:1rem 0 0 1rem;font-size:.58rem;padding:.42em .8em;gap:.4rem}
