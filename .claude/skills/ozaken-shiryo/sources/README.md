...
`lecture_cover/` は光のカーテン・背景の粒と筋・共通表紙のCSSと動作の正典です。便覧も装飾関数と `motion.css` を共有します。`template_gallery/next.py` に装飾を複製しません。

通常・週次資料は `publish.compose()` の最後、講演版はさらに専用CSSの後で `../scripts/apply_cover.py` を適用します。独自にHTMLを組む生成元でも `apply_cover.patch(page)` を最終段階に通します。全既存資料への適用は [全資料の表紙を統一する](../../../../docs/operations.md#全資料の表紙を統一する) を参照。本文を古い生成元で一括再組版する必要はありません。

表紙の「、」「。」は使わず、句点の区切りを改行で表します。本文へ機械的な一括置換を行いません。表紙にあった追加情報は、その直後へ常時表示のまま引き継ぎます。
