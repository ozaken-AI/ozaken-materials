...
window.TemplateMotion={mount(canvas){
 ...
 if(s.kind==='cutaway'){
  ...
  ['従来','変更後'].forEach((name,row)=>{const yy=60+row*93;label(name,x-10,yy+26,{'text-anchor':'end',class:'nx-compact'});let pos=0;[6,8,12,4].forEach((v,i)=>{const r=add('rect',{x:x+pw*pos/30,y:yy,width:pw*v/30,height:44,fill:colors[i]});const val=label(v+'h',x+pw*(pos+v/2)/30,yy+27,{class:'nx-inverse'});if(pw*v/30<27)val.style.opacity=0;if(row){rects.push(r);values.push(val);}pos+=v;});});
