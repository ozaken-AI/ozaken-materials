# 本文フラグメントの書き方

## 全体の形

Pythonで組み立てて、HTMLフラグメントを吐く。手書きしない。
既存の生成スクリプトを雛形にすると早い（`gen_credits.py` が最新かつ素直な例）。

```python
import os, sys
sys.path.insert(0, '/home/user/ozaken-materials/.claude/skills/ozaken-shiryo/scripts')
from domain_fig import fig_cols, fig_bars, fig_sheet, fig_ladder, fig_gap, fig_quad

BODY = []
A = BODY.append

A('<!--META title=タイトル ─ 副題 | desc=検索結果に出る一文。-->')
A(hero('Eyebrow ─ 分類', 'タイトル<br>2行目', 'リード文。3〜4行。'))
A(sec('sec-light', 'Section 01', '見出し', 'サブ', lede='…', fig=…, body=cards([...])))
...
A(close('締めの見出し', '締めの文章。'))

open('/tmp/body_foo.html', 'w', encoding='utf-8').write('\n'.join(BODY))
```

`hero()` `sec()` `close()` `cards()` は各生成スクリプトが持っている小さな関数。
新しく作るときは `gen_credits.py` からコピーして使う。

## 扉の部品

```python
hero(eyebrow, title, copy, cat=None, meta=BYLINE, toc=None)
```

| 部品 | 役割 | 上限（機械が弾く） |
|---|---|---|
| `eyebrow` | 助走の小さなラベル。罫線が自動で付く | 短く |
| `title` | 資料の題。`<br>` で**2行まで** | **1行の表示幅44**（全角22字） |
| `copy` | リード文 | **表示幅210**（3〜4行） |
| `toc` | 目次。長い資料だけ。4〜6の塊に畳む | — |

表示幅は全角2・半角1で数える。**字数ではない。**
上限は目安ではなく `build_page.check_hero()` が実際に弾く値で、
越えると `publish.py` が公開を止める。書く前に `hero_fit.py` で測れる。

題は演出が「」の中か英字のかたまりを自動で赤くするので、
そこを意識して言葉を置く。

リード文が上限に当たったときは、削るのではなく**置き場所を変える**。
「AとBとCを扱います」の列挙は `toc` の仕事で、リード文の仕事ではない。

## セクションの部品

```python
sec(tone, eyebrow, title, sub=None, lede=None, fig=None, body=None)
```

| 部品 | 役割 | 長さの目安 |
|---|---|---|
| `eyebrow` | 「Section 01」など小さなラベル | 短く |
| `title` | セクションの主張。**述語で言い切る** | 25文字前後 |
| `sub` | 見出しを1段かみ砕く | 40文字前後 |
| `lede` | 明朝体の大きめ導入文。要点を語る | 2〜4行。全セクションには置かない |
| `fig` | 図版。**毎セクション必ず1つ以上** | — |
| `body` | カード3枚 | 各2〜4行 |

`fig` には複数の図版を `+` で連結して渡せる。
情報量が多いセクションでは、棒グラフ＋表のように2枚重ねると理解が早い。

## 本文ブロック（blocks.py）

図版でもカードでもない、本文の中に置く型。**24種類ある**
（置くだけの17個と、押せる7個）。
実物の見本は、テンプレート便覧の Section 08〜13。

```python
from blocks import bignum, statgrid, meters, kpi_row, quote, lead_reveal, \
    callout, terms, steps, timeline, phases, split, checklist, faq, \
    marquee, spotlight, wave
A(sec(..., after=bignum('51', '施策', 'AI推進で打てる手の総数')))
```

**カードの外に置くので、`after=` に渡す。** `body=` に混ぜると
カードの格子に潰れて1列ぶんの幅になる（`take` と同じ事故）。

| 束 | 部品 | いつ使うか |
|---|---|---|
| 数字 | `bignum` | 比べる相手のいない数字を、画面いっぱいで |
| | `statgrid` | 独立した数字を3〜4つ並べる |
| | `meters` | 割合を、伸びる帯で |
| | `kpi_row` | 同じ値の前と後を、矢印でつなぐ |
| ことば | `lead_reveal` | 節の主張を1文で言い切る。**1面に1回だけ** |
| | `quote` | 他人の言葉。出典を示して引く（`take` とは別物） |
| | `callout` | 要点・落とし穴・補足の3種類だけ |
| | `terms` | 専門語に、その場で1行の説明を添える |
| 構造 | `steps` | 番号で進む手順。**5つを超えたら図版から移す** |
| | `timeline` | 日付で進むもの。件数が多いとき |
| | `phases` | 全体が何段階かを見せる。3〜5個 |
| | `split` | 優劣の無い2つ（`fig_versus` は赤い側が「注意すべき方」） |
| 一覧 | `checklist` | 可否の一覧。印は線2本で作るので書体に左右されない |
| | `faq` | 想定問答。**開閉させない** |
| 演出 | `marquee` | 全部は読ませなくてよい列挙 |
| | `spotlight` | 節の締め。**1面に1つまで** |
| | `wave` | 「動いている」ことだけを示す小さな印 |

### 押せる部品（7つ）

**常時の動きは足さない。触れたときだけ返す。**
投影中に動くものが増えると、本文が読めなくなる。

押されることが本当に起きるのは、**講演のあとに渡したURLを
参加者が自分の端末で開いたとき**。そこで効くものだけを置いてある。

| 部品 | いつ使うか |
|---|---|
| `copyable` | 依頼文・設定値をその場でコピー。**紙では成立しない** |
| `flip` | 表に問い、裏に答え。「考えてから開く」 |
| `poll` | 本文に置く1問。選ぶと正解と解説が出る |
| `tabs` | 経営・現場・情シスなど、立場で中身を切り替える |
| `toggle_pair` | Before / After を**同じ場所で**入れ替える |
| `accordion` | **投影では使わない。** 渡したあとに読む長い一覧を畳む |
| `counter` | 0から回る数字。押すともう一度回る |

**押した感触は ripple ひとつに統一する。** 部品ごとに違う返し方を
すると、押せるものと押せないものの区別が学習できない。
押せる箱には `oz-press` が付いていて、押した点から円が広がる。

**JSは `tpl_tail.html` にあり、document への委譲で動く。**
資料は暗号化されていて本文は復号のあとから差し込まれるので、
読み込み時に `querySelectorAll` で拾う書き方では動かない。

**図版と、どちらを使うか。**
投影して読ませるものは図版（SVGなので投影で字が潰れない、ただし文字数の上限がある）。
**長さが伸びるもの・件数が増えるものは本文ブロック**（HTMLとCSSなので自動で流れる）。

**部品を足すときは3か所。** `blocks.py` に関数、`tpl_style.html` の末尾にCSS、
そして**便覧に見本**。見本を足さないと、次にその形が要る場面で誰も見つけられない。

**新しいCSSは、既存の資料には届かない。** `tpl_style.html` は組版のときに
差し込まれるので、既に公開した資料は `publish.py --update` で焼き直すまで
古いCSSのまま。新しいブロックを既存資料で使うなら、焼き直しが要る。

## カードの書き方

```python
cards([
    ('<span class="mth">短いラベル</span>見出しになる一文',
     '本文。2〜4行。数字と固有名詞を入れる。'),
    ...
])
```

`mth` ラベルは、聴講者が話の位置を見失わないための目印。
「前提条件」「落とし穴」「判断軸」「試算」のように、**そのカードの役割**を書く。
内容の要約ではない。

カードの本文には `<b>` が使える（SVGの中では使えない）。
効かせどころは1セクションに1箇所まで。

## 構成の型

### 解説型（製品・技術・制度の理解）
1. なぜ今それが問題か（`fig_gap` で対比）
2. 全体像・分類（`fig_cols`）
3. 中核の数値（`fig_sheet` / `fig_bars`）
4. 仕組みの分解（`fig_cols` / `fig_context`）
5. 実例・シナリオ（`fig_bars` + `fig_sheet`）
6. 見積り・手順（`fig_ladder`）
7. 境界線・誤解（`fig_quad`）
8. 周辺領域（`fig_cols`）
9. 落とし穴と行動（`fig_ladder`）
→ 締め

### 実践型（業務での使い方）
1. 現場で起きていること
2. そもそも何か（定義の整理）
3. 3つの型（`fig_cols`）
4. 型ごとの詳細 ×3
5. 使い分けの判断（`fig_quad`）
6. 導入の順序（`fig_stairs`）
7. つまずきどころ
→ 締め

### 講演セッション型（AX Table のような1コマ用）
本文は5〜7本と短くする。1セッション30分なら、
図版7点・カード15枚が上限。それ以上は喋りきれない。

## 締めのセクション

`close(title, copy)` は navy 一色。図版は置かない。
**事実の要約をしない。** 資料全体を1段上から言い直す。

良い例（Copilot Credits の資料より）:

> この変化の本質は、値上げでも値下げでもない。
> **AIの使い方が、はじめて金額として見えるようになった**ということだ。
> 重い仕事を1本回せば$7、軽い要約なら$1。
> この感覚が現場に共有されると、「とりあえず全部AIに」という発想は自然に消え、
> 「ここは人がやったほうが速い」「この仕事は$7払う価値がある」という判断が生まれる。

事実（$7と$1）を、判断の話に翻訳している。ここが資料の価値になる。

## 出典の扱い

外部資料を元にしたときは、**図版のキャプションに出典を書く**。

```
'出典: Microsoft Copilot Credits Guide（2026年8月）。金額は $0.01/クレジットで換算'
```

原典の数値と、自分で計算した値を必ず区別する。
「〜と明記されている」「〜は参考値」といった書き分けを本文でも行う。
壇上で「ここは私の試算です」と言えることが、資料の信用を支える。

## 既存資料を直すとき

本文だけ差し替えれば、外側のロック画面と鍵はそのまま使える。

```python
import lockbox
inner = lockbox.decrypt(path, MASTER)
# inner を編集
lockbox.encrypt(path, MASTER, inner)      # 鍵はすべて維持される
assert lockbox.decrypt(path, MASTER) == inner
```

`lockbox.create()` は鍵を作り直すので、**パスワードを変える意図がないときは使わない**。
図版だけ作り直す場合も、生成スクリプトから組み直して `encrypt` で戻すのが安全。
