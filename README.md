...
最初に [AGENTS.md](./AGENTS.md) を読んでください。作成・改訂・確認・公開までの手順は [リポジトリ運用手順](./docs/operations.md)、講演と印刷の仕様は [デザイン規約](./docs/lecture-design.md) にまとめています。

現在のテンプレート便覧は `build_template_gallery.py`、講演版の `use-to-delegate.html` は `delegation_pilot/build.py` から生成します。旧 `gen_template.py` / `gen_use_to_delegate.py` だけでは現在のデザインを再現できません。生成元の対応とコマンドは [運用手順](./docs/operations.md#編集する場所) を参照してください。
...
| `10_policy/` | 政策はどう変わるか | 日本・米国・中国・EUのAI政策 |
| `11_stats/` | 数字から何が分かるか | AI関連統計 |
| `12_jobs/` | 仕事に何が起きるか | 雇用・仕事の変化 |
...
先頭の2桁の連番は分類の並び順を持たせるために残しています。走査は
`[0-9][0-9]_*` を基本とし、番号と `_` は外さないでください。古いスクリプトに
`0*_*` が残っている場合は、10以降の分類を落とさないか確認します。
...
生成元を修正し、対象を再生成・検証してからGitへ記録します。
Cloudflare Pagesは `main` の更新で本番を配信し直します。
ローカル生成、暗号化、コミット、本番公開を区別してください。
公開の許可・差分の選び方・公開後の確認は [運用手順](./docs/operations.md#gitへの記録と本番反映) に従います。

共通アセットを変更した場合は、Gitへ記録する前に参照ハッシュを揃えます。
